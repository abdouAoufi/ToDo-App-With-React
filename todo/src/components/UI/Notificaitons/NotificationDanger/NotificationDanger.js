import React, { Component } from "react";
import "./Notification.css";
import CSSTransition from "react-transition-group/Transition";

class Notificaion extends Component {
  state = {
    isDisplay: true,
  };

  closeHandler = () => {
    let currentState = this.state.isDisplay;
    this.setState({ isDisplay: !currentState });
  };

  render() {
    let classes = [
      this.state.isDisplay ? "NotOpen" : "NotClose",
      "fixed",
      "top-20",
      "right-10",
      "w-1/4",
    ]

    return (
      <div className={classes.join(" ")}>
        <div
          className="bg-white border-l-4 border-orange-500 text-orange-700 p-4"
          role="alert"
        >
          <p className="font-bold text-red-600">{this.props.WarTitle}</p>
          <span className="absolute top-0 bottom-0 right-0 px-4 py-3">
            <svg
              onClick={this.closeHandler}
              className="fill-current h-6 w-6 text-red-500"
              role="button"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
            >
              <title>Close</title>
              <path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z" />
            </svg>
          </span>
          <p className="text-red-400">{this.props.WarBody}</p>
        </div>
      </div>
    );
  }
}

export default Notificaion;
